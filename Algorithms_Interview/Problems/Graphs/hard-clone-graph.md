...
public class Solution {
    private IDictionary<Node, Node> _clones = new Dictionary<Node, Node>();
    
    public Node CloneGraph(Node node) {
        if (node == null) return null;
        if (_clones.ContainsKey(node)) return _clones[node];
        
        _clones[node] = new Node(node.val);
        
        foreach (var nei in node.neighbors) {
            _clones[node].neighbors.Add(CloneGraph(nei));
        }
